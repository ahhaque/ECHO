...
public class Miner implements OptionHandler{
    ...
    void printResult(int chunkSize, int K, int Minpts)
    {
        ...
        try{
            ...
            {                
                 out = new BufferedWriter(new FileWriter("SAND_"+ Constants.VERSION +"-"+InFile.substring(0,4)+"-Tl"+this.Tl+
                 "-Tc"+this.Tc+"-S"+this.ChunkSize+"-N"+K+"-L"+M+"-M"+Minpts+"-"+ ClassifierName+".tmpres"));
                 out.write("Chunk#\tFP\tFN\tNC\tErr\tGlobErr\n" );
            }
            //else
            {
                 out = new BufferedWriter(new FileWriter("SAND_"+ Constants.VERSION +"-"+InFile.substring(0,4)+"-Tl"+this.Tl+
                 "-Tc"+this.Tc+"-S"+this.ChunkSize+"-N"+K+"-L"+M+"-M"+Minpts+"-"+ ClassifierName+".tmpres",true));
    ...
    {
        try{
            BufferedWriter    out = new BufferedWriter(new FileWriter("SAND_"+ Constants.VERSION +"-"+InFile.substring(0,4)+"-Tl"+this.Tl+
                 "-Tc"+this.Tc+ "-S"+this.ChunkSize+"-N"+K+"-L"+M+"-M"+Minpts+"-"+ ClassifierName+".res"));
  ...
  public Enumeration listOptions() {
    ...
    newVector.
	addElement(new Option("\tSet chunk size" +
			      "\t(default 1000)",
			      "S", 1, "-S <chunk size>"));
    newVector.
  ...
  public String [] getOptions() {
    ...
    public static void main(String[] args)
    {
        ...
        try{
         fa= new FileAppender(sl, "SAND_"+Constants.VERSION+"-"+ev.InFile.substring(0,4)+"-Tl"+ev.Tl+
                 "-Tc"+ev.Tc+"-S"+ev.ChunkSize+"-N"+ev.K+"-L"+ev.M+"-M"+ev.Minpts+"-"+ ev.ClassifierName+".log",false);
        }
        catch(IOException e)
        {
            System.out.println(e.getMessage());
