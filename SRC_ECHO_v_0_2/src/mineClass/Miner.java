    ...
    void printResult(int chunkSize, int K, int Minpts)
    {
        ...
        try{
            ...
            {                
                 ...
                 "-C"+this.Tc+ "-S"+this.ChunkSize+"-U"+Constants.TAU+"-D"+Constants.DYNAMIC+"-W"+Constants.MAXSLIDINGWINDOWSIZE+"-A"+Constants.SENSITIVITY+"-G"+Constants.GAMMA+"-R"+Constants.RELAXATION + ".tmpres"));
                 out.write("Chunk#\tFP\tFN\tNC\tErr\tGlobErr\n" );
            }
            //else
            {
                 out = new BufferedWriter(new FileWriter("ECHO_"+ "V" + Constants.VERSION +"-"+(InFile.lastIndexOf("/")==-1?InFile.substring(InFile.lastIndexOf("\\")+1):InFile.substring(InFile.lastIndexOf("/")+1))+"-T"+this.Tl+
                 "-C"+this.Tc+ "-S"+this.ChunkSize+"-U"+Constants.TAU+"-D"+Constants.DYNAMIC+"-W"+Constants.MAXSLIDINGWINDOWSIZE+"-A"+Constants.SENSITIVITY+"-G"+Constants.GAMMA+"-R"+Constants.RELAXATION+".tmpres",true));
    ...
    void printResult(String finit, int totInstance,int K, int M)
    {
        try{
            BufferedWriter out = new BufferedWriter(new FileWriter("ECHO_"+ "V" + Constants.VERSION +"-"+(InFile.lastIndexOf("/")==-1?InFile.substring(InFile.lastIndexOf("\\")+1):InFile.substring(InFile.lastIndexOf("/")+1))+"-T"+this.Tl+
                 "-C"+this.Tc+ "-S"+this.ChunkSize+"-U"+Constants.TAU+"-D"+Constants.DYNAMIC+"-W"+Constants.MAXSLIDINGWINDOWSIZE+"-A"+Constants.SENSITIVITY+"-G"+Constants.GAMMA+"-R"+Constants.RELAXATION+".res"));
    ...
    public static void main(String[] args)
    {
        ...
        try{
         fa= new FileAppender(sl, "ECHO_"+ "V" + Constants.VERSION +"-"+(ev.InFile.lastIndexOf("/")==-1?ev.InFile.substring(ev.InFile.lastIndexOf("\\")+1):ev.InFile.substring(ev.InFile.lastIndexOf("/")+1))+"-T"+ev.Tl+
                 "-C"+ev.Tc+ "-S"+ev.ChunkSize+"-U"+Constants.TAU+"-D"+Constants.DYNAMIC+"-W"+Constants.MAXSLIDINGWINDOWSIZE+"-A"+Constants.SENSITIVITY+"-G"+Constants.GAMMA+"-R"+Constants.RELAXATION+".log",false);
        }
        catch(IOException e)
        {
            System.out.println(e.getMessage());
